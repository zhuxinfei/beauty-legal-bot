...
function buildPythonScript(spec, { pageTimeoutMs = 20000, outputPath = '' } = {}) {
  return `
import asyncio
import json
import os
        ...
        async with AsyncWebCrawler(config=browser_config, base_directory=base_directory) as crawler:
            for item in spec:
                ...
                try:
                    ...
                    text_value = markdown or fit_markdown or getattr(result, "text", "") or ""
                    results.append({
                        "url": url,
                        "final_url": final_url,
                        "title": title,
...
async function main() {
  const { input, output, python, baseDir, limit, pageTimeoutMs } = parseArgs(process.argv);
  if (!input) {
  ...
  const stdout = execFileSync(python, ['-c', buildPythonScript(spec, { pageTimeoutMs, outputPath: output ? resolve(output) : '' })], { encoding: 'utf8', env, maxBuffer: 1024 * 1024 * 200 });
