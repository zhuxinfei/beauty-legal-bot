...
每周从用户维护的美妆法务信息源目录中抓取公开网页候选，并结合公众号线索，用 DeepSeek 生成面向美妆公司法务人员的情报周报。
...
| 模块组织 | 按新规/案例/美妆动态/IP/进出口五大模块展示，宁缺毋滥 |
| 飞书摘要 | 只展示本周重点风险和“动作：...” |
...
- 公众号不执行 fetch，不算抓取失败。
- 公众号会进入 `leads[]`，只作为 DeepSeek 判断选题方向的线索。
- DeepSeek 不得把公众号线索作为事实来源。
- 没有公开原文链接的内容不能进入最终报告。
...
- 飞书推送以最终报告条目指纹做 30 天历史去重；若全部条目已推送过，则只更新页面不重复打扰。
