...
程序通过钉钉群机器人发送一条原生 Markdown 周报，依次包含管理层摘要、分板块专业正文和本期结论。每条事项保留事实、法务研判、业务影响、行动或观察点、法定节点以及可点击原文；全部文字可直接复制。中国信息优先，行动事项与持续观察统一编号。

没有高质量内容的模块不会显示。消息上限固定为 18,000 UTF-8 字节；超限时先压缩低优先级、非中国事项，再在确有必要时省略最低优先级事项并标注数量，至少保留一条可核验来源。消息不会拆成多张卡片。
...
GitHub Actions 还需要 `CLOUDFLARE_API_TOKEN`，用于部署 Worker；账号 ID 和 KV namespace ID 配置在 workflow 环境变量中。
...
Worker 会运行完整周报管道，并通过钉钉群机器人发送一条完整 Markdown 周报；未配置钉钉时回退飞书摘要。可靠来源覆盖不足或钉钉发送失败会让整轮任务失败。
...
- 默认模型为 `deepseek-chat`；旧 DeepSeek 凭据和模型回退已停用。使用 `deepseek-chat` 时正式请求默认采用中等推理强度；高推理强度仅显式启用。
...
- 协作平台推送以最终报告条目指纹做 30 天历史去重；若全部条目已推送过，则不重复打扰。
- AI 输出仅供法务信息初筛，不构成正式法律意见。
- 推送顺序固定为：生成并校验原生 Markdown → 发送唯一一条钉钉消息 → 成功后写入去重状态。
