...
| 模块组织 | 六大模块持续采集和分析；钉钉正文只展开本周有高质量行动事项的模块 |
| 来源恢复 | 直接请求失败后，对网络、超时、429 和 5xx 做有界重试；必要时用无头浏览器读取公开页面，最后尝试已核验的同机构官方备用地址 |
| 来源门槛 | 中国高优先级可抓取源和全部可抓取源均至少 90% 覆盖；未达门槛时不进入 AI 和推送阶段 |
| 协作平台推送 | 钉钉群机器人只发送一条 Markdown 周报；未配置钉钉时可回退飞书摘要 |
| 管理层看板 | 生成 1080×1440 的中国优先行动看板，只展示 Top 3 核心判断和 Top 3 优先行动 |
| 群内版式 | 正文按核心判断、优先行动、有效模块和持续观察组织；单条消息压缩到 18,000 UTF-8 字节以内 |
...
程序通过钉钉群机器人发送一条完整 Markdown 周报。正文依次包含行动看板、本周核心判断、优先行动、有合格行动事项的业务模块、持续观察和来源说明；中国信息优先。

没有高质量内容的模块不会显示。消息上限固定为 18,000 UTF-8 字节，超限时先移除低优先级关注事项，再压缩或省略低分行动事项的重复展开；Top 3 核心判断、优先行动和来源链接优先保留。
...
主分析和救援分析均未发现重要事项时，钉钉发送一条明确的“本期无重大合规更新”文字简报，不生成空行动看板。只要 AI 曾产出条目但全部在技术过滤环节归零，流程会以失败退出并停止推送，避免把过滤异常误报为正常零更新。
