...
4. `CLOUDFLARE_API_TOKEN`，用于部署 Worker 和发布高清资讯长图；缺失时自动发送完整文字版

GitHub Actions 不需要 `DINGTALK_CLIENT_ID`、`DINGTALK_CLIENT_SECRET`、`DINGTALK_OPERATOR_ID` 或 `DINGTALK_WORKSPACE_ID`。

## 推送版式

程序每轮只发送一条 Markdown 消息，内容顺序为：管理层摘要、高清资讯长图、本期结论和编号来源索引。图片不可用时，同一条消息自动切换为包含事实、法务研判、业务影响、行动或观察点、法定节点和来源的完整文字版。

中国信息整体优先，再展示其他国家和地区；同一层级继续按质量、风险和直接相关性排序。没有高质量内容的模块不显示，也不填充低质量资讯。

消息最大为 18,000 UTF-8 字节。长图版用编号来源索引控制长度；文字回退版先压缩字段，再在必要时省略最低分条目并标注数量。不会拆成多个卡片，中国优先顺序和所有保留条目的原文链接必须保留。

资讯图片使用 1080 CSS 像素宽、2 倍像素密度和动态高度。每次推送使用 `/assets/editorial-report/YYYY-MM-DD.png` 日期版 URL；`latest` 仅用于运维诊断。
...
- 图片会先写入日期版 Cloudflare KV，并从公开 URL 回读确认是有效 PNG；任何图片错误都会回退完整文字版。
- 只有钉钉实际发送成功后才写入 30 天去重记录，图片是否可用不影响这一原则。
